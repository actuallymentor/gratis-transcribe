...
- The app's production CSP includes `connect-src 'self'`, so the same-origin app-update reachability probe is allowed under the strict Cloudflare Pages headers.
- Treat app-update cache deletion as best-effort; after the online probe has passed, the app should still reload and re-register even if an individual Cache API delete rejects.
