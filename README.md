...
Settings includes an `Update app` command. It checks that the app origin is reachable, unregisters active service workers, clears app-shell and WASM runtime caches, and reloads the page so the current service worker installs again. It preserves downloaded speech models, source audio, and transcripts.
